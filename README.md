<div align="center">

<img src="https://github.com/user-attachments/assets/86c3ac87-4433-4b42-b718-d4ad4785869f" alt="IIT Roorkee Logo" width="110"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<img src="https://github.com/user-attachments/assets/900b54b7-2cc6-4ba6-a8ce-0949e3f6ea93" alt="IIMT University Logo" width="110"/>

# 🧬 IIMT Workshop – Module 4

## AI/ML for Antimicrobial Peptide Engineering

**Student Training Programme**  
**Department of Biotechnology, IIMT University, Meerut**

In collaboration with the **Structural and Protein Dynamics Laboratory (Hazra Group)**  
**Indian Institute of Technology Roorkee**

<br>

<img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white">
<img src="https://img.shields.io/badge/Google%20Colab-F9AB00?style=for-the-badge&logo=googlecolab&logoColor=white">
<img src="https://img.shields.io/badge/Hugging%20Face-FFD21E?style=for-the-badge&logo=huggingface&logoColor=black">

<br><br>

<img src="https://img.shields.io/badge/Protein%20Language%20Models-7B1FA2?style=for-the-badge">
<img src="https://img.shields.io/badge/ProtBERT-5E35B1?style=for-the-badge">
<img src="https://img.shields.io/badge/CD--HIT-00897B?style=for-the-badge">
<img src="https://img.shields.io/badge/BioPython-2E8B57?style=for-the-badge">

<br><br>

<img src="https://img.shields.io/badge/scikit--learn-F7931E?style=for-the-badge&logo=scikitlearn&logoColor=white">
<img src="https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white">
<img src="https://img.shields.io/badge/NumPy-013243?style=for-the-badge&logo=numpy&logoColor=white">
<img src="https://img.shields.io/badge/PyTorch-EE4C2C?style=for-the-badge&logo=pytorch&logoColor=white">

</div>

---

# Workshop Overview

This repository contains all teaching material, practical notebooks, datasets, and reference resources for the **IIMT Workshop on AI/ML for Antimicrobial Peptide Engineering**. The workshop introduces participants to computational peptide engineering, bioinformatics, protein language models, and machine learning approaches for antimicrobial peptide discovery.

---

# Workshop Banner

<p align="center">
<img src="https://github.com/user-attachments/assets/cb0d9421-ba27-4ab3-a297-d2c04ff9838b" width="90%">
</p>

<p align="center">
<img src="https://github.com/user-attachments/assets/2520ec19-bef2-47e8-86c1-b1555f9f0575" width="90%">
</p>

---

# Overall Workflow

<p align="center">
<img src="https://github.com/user-attachments/assets/ad52a0c3-8411-4dab-bae8-9d5e202f9115" width="95%">
</p>

---

# Repository Structure

```text
IIMT-WORKSHOP/
├── Day-1/
│   ├── Theory_day_1.ppt
│   └── Practicle_session_1.ipynb
│
├── Day-2/
│   ├
│   └── Practicle_session_2.ipynb
│
├── Day-3/
│   ├
│   └── Practicle_session_3.ipynb
│
├── references/
│   └── Research papers & reading material
│
├── LICENSE
├── README.md
└── .gitignore
```

---

# Getting Started

```bash
git clone https://github.com/rikmicrobio/IIMT-WORKSHOP-.git

cd IIMT-WORKSHOP-
```

Each `Day-X` folder contains:

- 📖 Theory presentation
- 💻 Hands-on Colab/Jupyter notebook
- 📂 Supporting datasets (where applicable)

The `references/` folder contains additional literature for further reading.

---

## 👥 Contributers:

| Name | Position | Affiliation |
|------|----------|-------------|
| **Prof. Saugata Hazra** | Professor & Principal Investigator | Hazra Group, Department of Biosciences and Bioengineering (BSBE), IIT Roorkee |
| **Dr. Rik Ganguly** | Postdoctoral Fellow | Hazra Group, Department of Biosciences and Bioengineering (BSBE), IIT Roorkee |
| **Siddhant Poudyal** | Project Assistant | Hazra Group, Department of Biosciences and Bioengineering (BSBE), IIT Roorkee |
| **Arghyadeep Das** | B.Tech Biotechnology | Adamas University |
---
## 📬 Contact

- **Name:** Dr. Rik Ganguly
- **Email:** rikgangulybioinfo@gmail.com
- **Phone:** +91 63504 33960
- **LinkedIn:** https://www.linkedin.com/in/rik-ganguly-70a7a197/
# References

1. Wang, H., Lei, Z., Zhang, X., Zhou, B., & Peng, J. (2016). *Machine Learning Basics*. In **Deep Learning** (pp. 98–164). Springer.

2. Li, W., & Godzik, A. (2006). **CD-HIT:** A fast program for clustering and comparing large sets of protein or nucleotide sequences. *Bioinformatics*, **22**(13), 1658–1659.

3. Elnaggar, A., Heinzinger, M., Dallago, C., Rehawi, G., Wang, Y., Jones, L., Feher, T., Angerer, C., Steinegger, M., Bhowmik, D., & Rost, B. (2021). *ProtTrans: Towards Cracking the Language of Life's Code Through Self-Supervised Deep Learning and High Performance Computing.*

4. ProtBERT (Rostlab): https://huggingface.co/Rostlab/prot_bert

---

<div align="center">
Linus Pauling: "The eventual goal... is the description of these activities in terms of their structure, i.e. the spatial distribution of their constituent atoms."

</div>
